  ...
  _renderUpgradeButtons(bundleId) {
    ...
    return `
      <div class="bundle-upgrade-controls" onclick="event.stopPropagation();" style="display:flex;flex-direction:row;align-items:center;gap:3px;margin-right:4px;">
        <button class="bundle-upgrade-btn ${canDowngrade ? '' : 'disabled'}" style="width:20px;height:20px;border-radius:4px;border:1px solid ${canDowngrade ? 'var(--primary)' : '#444'};background:${canDowngrade ? 'var(--primary)' : 'transparent'};color:${canDowngrade ? '#fff' : '#666'};display:flex;align-items:center;justify-content:center;padding:0;cursor:${canDowngrade ? 'pointer' : 'not-allowed'};font-size:8px;" ${canDowngrade ? `onclick="RecommendPage.downgradeBundle('${bundleId}')"` : ''} title="降级">
          <svg viewBox="0 0 12 12" width="10" height="10"><polygon points="6,2 10,9 2,9" fill="currentColor"/></svg>
        </button>
        <button class="bundle-upgrade-btn ${canUpgrade ? '' : 'disabled'}" style="width:20px;height:20px;border-radius:4px;border:1px solid ${canUpgrade ? 'var(--primary)' : '#444'};background:${canUpgrade ? 'var(--primary)' : 'transparent'};color:${canUpgrade ? '#fff' : '#666'};display:flex;align-items:center;justify-content:center;padding:0;cursor:${canUpgrade ? 'pointer' : 'not-allowed'};font-size:8px;" ${canUpgrade ? `onclick="RecommendPage.upgradeBundle('${bundleId}')"` : ''} title="升级">
          <svg viewBox="0 0 12 12" width="10" height="10"><polygon points="6,10 10,3 2,3" fill="currentColor"/></svg>
        </button>
      </div>
    `;
  },
    ...
    const lightingDemo = bundle.id === 'lighting'
      ? `<div class="lighting-demo-wrapper" style="margin:12px 0;border:1px solid var(--border,#333);border-radius:12px;overflow:hidden;background:#0a0a10;">
           <div style="height:600px;">
             <iframe src="file:///D:/TRAE_SOLO_Data/nietest/dengguangzhanshi/smart-lighting-demo/smart-lighting-demo.html" style="width:100%;height:100%;border:none;" scrolling="no"></iframe>
           </div>
         </div>`
      : '';
